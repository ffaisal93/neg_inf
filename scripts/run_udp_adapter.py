...
def main():
    ...
    if data_args.task_name!="udp_all" and training_args.do_train:
    ...
    elif data_args.task_name=="udp_all" and training_args.do_train:
        dataset = load_from_disk(data_args.data_file)
        dataset=dataset.train_test_split(test_size=0.2)
        dataset = DatasetDict({"train":dataset['train'],"validation":dataset['test']})
        dataset = preprocess_dataset(dataset, tokenizer, labels, data_args, pad_token_id=-1)
        # print(dataset)
        # dataset = load_dataset("universal_dependencies", 'en_ewt',split=['train[:10]','validation','test'], cache_dir=model_args.cache_dir)
