...
def main():
    ...
    if task_name!='tydiqa_all' and training_args.do_train:
    ...
    elif task_name=='tydiqa_all' and training_args.do_train:
    ...
    if training_args.do_predict:
        ...
        def get_dataset(data_lang):
            predict_examples = raw_datasets["validation"].filter(lambda example: example['id'].startswith(data_lang))
            if data_args.max_predict_samples is not None:
                # We will select sample from whole data
                predict_examples = predict_examples.select(range(data_args.max_predict_samples))
            # Predict Feature Creation
        ...
        for lang, info in adapter_info.items():
            ...
            try:
                predict_dataset,predict_examples=get_dataset(info)
