...
def main():
    ...
    if data_args.task_name=="pos_all":
        raw_datasets = load_from_disk(data_args.data_file)
    ...
    # if training_args.do_predict:
        ...
        #     predict_dataset = load_dataset("scripts/universal_dependencies.py", data_lang,
        #                                    split=['test'], cache_dir=model_args.cache_dir)
        #     predict_dataset = DatasetDict({"test":dataset[0]})
    ...
    # Metrics
    if data_args.prefix!=False:
        metric = load_metric("seqeval",cache_dir=model_args.cache_dir+"/"+data_args.prefix)
    else:
        metric = load_metric("seqeval",cache_dir=model_args.cache_dir)
    ...
    if training_args.do_predict:
        ...
        for lang, info in adapter_info.items():
            ...
            try:
                predict_dataset=get_dataset(info["full"])
                ...
                logger.info("%s,%s,%s,%s,%s\n" % (data_args.prefix,task_name, lang, metrics['predict_f1'], metrics['predict_accuracy']))
